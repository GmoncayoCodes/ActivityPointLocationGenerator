...
#   Run R script to decrypt Google Location History (GLH) JSON file
#   This script have to run from code directory in the project folder
...
try:
    ...
    # NOTE: Set code folder as your python working directory
    # =============================================================================
    urlScriptDirectory = os.getcwd()
    if urlScriptDirectory.endswith('code') :
        urlWorkingDirectory = os.path.abspath(os.path.join(urlScriptDirectory, os.pardir))
        urlDataTransformed = urlWorkingDirectory + '\\dataTransform\\'
        urlDataFinal = urlWorkingDirectory + '\\dataFinal\\'
        urlLog = urlWorkingDirectory + '\\log\\'
        ...
        #Read RData file        
        rDataFileName = os.listdir(urlDataTransformed)[0]        
        idFile = rDataFileName[0:len(rDataFileName)-6]
        urlFile = urlDataTransformed + rDataFileName
        fileObj = Path(urlFile)
        if(fileObj.is_file()):
            dataFromR = pyreadr.read_r(urlFile)
            dfSource = pd.DataFrame(dataFromR['dataFrameSource'])
            df = dfSource
            df['idFile'] = idFile
            df = df[['idFile','timestampMs', 'longitudeE7', 'latitudeE7']]
            msg = 'Processing: ' + rDataFileName + ' IdFile: ' + idFile 
            print(msg)
            logging.info(msg)
            ...
            if len(df) >= MIN_NUMBER_OBSERVATIONS:
                dfSkmob = skm.TrajDataFrame(df, user_id='idFile', datetime='timestampMs', longitude='longitudeE7', latitude='latitudeE7')
                ...
                dfFinalSummary.to_csv(urlDataFinal + exportFile + '.csv', header=True, index = False)
                dfFinalSummary.to_pickle(urlDataFinal + exportFile + '.dat')
                msg = "Computation succed: {} contains {} APL".format(rDataFileName, len(dfFinal))
                print(msg)
                logging.info(msg)
                
            else:
              msg = "File {} has few observations: {}".format(rDataFileName, len(df))
              print(msg)
              logging.warning(msg)    
        else:
             msg = 'No File named {}. Check your dataDecrypted directory. Don\'t forget to copy script in code directory and RData file in dataDecrypted directory'.format(rDataFileName)
             print(msg)
             logging.warning(msg)
        ...
        msg = "Can't execute Script. Check Working Directory!! Your current directory is {}".format(urlScriptDirectory)
        print(msg)
except:
